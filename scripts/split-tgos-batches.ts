/**
 * Split TGOS batch CSV into multiple files (max 10,000 records per file)
 *
 * TGOS has a daily limit of 10,000 records, so we need to split large batches.
 *
 * Usage: bun scripts/split-tgos-batches.ts
 */

const BATCH_SIZE = 10000;
const INPUT_FILE = 'data/taichung-tgos-batch.csv';
const OUTPUT_PREFIX = 'data/taichung-tgos-batch';
...
async function splitCSV() {
  ...
  const manifestPath = 'data/taichung-tgos-batches-manifest.json';
